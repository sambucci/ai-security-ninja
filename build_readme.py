...
The site is the source of truth. This script fetches the public JSON files the
site itself renders from (https://aisecurity.ninja/data/*.json), and writes a
README in list form. Output is deterministic: same data in, same bytes out, so
the monthly workflow only commits when something on the site actually changed.

Usage:
    python3 build_readme.py                 # fetch from the live site
    python3 build_readme.py --local DIR     # read DIR/*.json instead (testing)
...
def main() -> int:
    ...
    out.append(f"The site is the source of truth. This README is generated by [`build_readme.py`](build_readme.py) "
               f"from `{SITE}/data/*.json`, and a [monthly workflow](.github/workflows/rebuild.yml) rebuilds it, "
